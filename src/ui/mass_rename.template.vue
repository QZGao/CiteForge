<template>
	<div class="citeforge-mass-rename-shell" v-if="open">
		...
		<div class="citeforge-mass-rename-window" role="dialog" aria-modal="true"
			:aria-label="t('ui.massRename.dialog.title')">
			<header class="citeforge-mass-rename-header">
				...
				<div class="citeforge-mass-rename-header-actions">
					<cdx-button weight="quiet" size="small" @click.prevent="closeDialog">{{ t('ui.default.close')
					}}</cdx-button>
				</div>
			</header>
			<div class="citeforge-mass-rename-body">
				<section class="citeforge-mass-rename-controls">
					<cdx-field class="citeforge-mass-rename-field">
						<cdx-multiselect-lookup v-model:input-chips="fieldChips" v-model:selected="fieldSelection"
							v-model:input-value="fieldInput" :menu-items="fieldMenuItems" :menu-config="fieldMenuConfig"
							:placeholder="t('ui.massRename.dialog.addNamingPartsPlaceholder')" @input="onFieldInput"
					...
					<div class="citeforge-mass-rename-rowstack">
						...
						<cdx-select class="citeforge-mass-rename-select" :menu-items="incrementOptions"
							v-model:selected="config.incrementStyle" />
					</div>
					<div class="citeforge-mass-rename-rowstack">
						<span class="citeforge-mass-rename-label">{{ t('ui.massRename.dialog.delimiter') }}</span>
						<cdx-text-input class="citeforge-mass-rename-input" v-model="config.delimiter"
					...
					<div class="citeforge-mass-rename-rowstack">
						<span class="citeforge-mass-rename-label">{{ t('ui.massRename.dialog.spaceReplacement')
						}}</span>
						<cdx-text-input class="citeforge-mass-rename-input" v-model="config.replaceSpaceWith"
							placeholder="_" :aria-label="t('ui.massRename.dialog.spaceReplacement')" />
					</div>
					<div class="citeforge-mass-rename-rowstack citeforge-mass-rename-rowstack--wrap">
						<cdx-checkbox v-model="config.convertYearDigits">{{ t('ui.massRename.dialog.convertYearDigits')
						}}</cdx-checkbox>
					</div>
					<div class="citeforge-mass-rename-rowstack citeforge-mass-rename-rowstack--wrap">
						<cdx-checkbox v-model="config.lowercase">{{ t('ui.massRename.dialog.lowercase')
						}}</cdx-checkbox>
						<cdx-checkbox v-model="config.stripDiacritics">{{ t('ui.massRename.dialog.stripDiacritics')
						}}</cdx-checkbox>
						<cdx-checkbox v-model="config.stripPunctuation">{{ t('ui.massRename.dialog.stripPunctuation')
						}}</cdx-checkbox>
					</div>
				...
				<section class="citeforge-mass-rename-list">
					<div class="citeforge-mass-rename-toolbar">
						<input type="search" class="citeforge-mass-rename-search"
							:placeholder="t('ui.massRename.dialog.filterPlaceholder')" :value="query"
							@input="onQueryInput" />
					...
					<div class="citeforge-mass-rename-table">
						<div class="citeforge-mass-rename-row citeforge-mass-rename-head">
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--check">
								<cdx-checkbox :indeterminate="selectAllIndeterminate" :model-value="selectAllChecked"
							...
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--name">{{
								t('ui.massRename.dialog.colReference') }}</div>
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--uses">{{
								t('ui.massRename.dialog.colUses')
							}}</div>
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--suggest">{{
								t('ui.massRename.dialog.colSuggestedName') }}</div>
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--actions"></div>
						</div>
						<div v-for="row in filteredRows" :key="row.ref.id" class="citeforge-mass-rename-row"
							...
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--name">
								<div class="citeforge-mass-rename-name">
									<span class="citeforge-mass-rename-name__label">{{ row.ref.name ||
										t('ui.default.nameless')
									}}</span>
									<span class="citeforge-mass-rename-name__bucket">{{ bucketFor(row) }}</span>
								</div>
								<div class="citeforge-mass-rename-snippet">
									<template
										v-for="(segment, idx) in linkifyOrFallback(row.snippet, t('ui.massRename.dialog.noInlineContent'))"
										:key="`mass-snippet-${row.ref.id}-${idx}`">
								...
								<div class="citeforge-mass-rename-meta">
									<span v-if="row.metadata.author || row.metadata.last || row.metadata.first"
									...
									<span v-if="row.metadata.title" class="citeforge-mass-rename-chip">{{
										t('ui.massRename.dialog.metaTitle') }} {{ row.metadata.title }}</span>
									<span v-if="row.metadata.work" class="citeforge-mass-rename-chip">{{
										t('ui.massRename.dialog.metaWork') }} {{ row.metadata.work }}</span>
									<span v-if="row.metadata.domain" class="citeforge-mass-rename-chip">{{
										t('ui.massRename.dialog.metaWebsite') }} {{ row.metadata.domain }}</span>
									<span v-if="row.metadata.dateDisplay || row.metadata.year || row.metadata.textYear"
										class="citeforge-mass-rename-chip">
										{{ t('ui.massRename.dialog.metaDate') }} {{ row.metadata.dateDisplay ||
											row.metadata.year ||
											row.metadata.textYear }}
									</span>
								</div>
							</div>
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--uses">{{
								row.ref.uses.length }}</div>
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--suggest">
								<cdx-text-input :disabled="!row.active" :model-value="row.suggestion"
							...
							<div class="citeforge-mass-rename-col citeforge-mass-rename-col--actions">
								<cdx-button size="small" weight="quiet" @click.prevent="regenerateRow(row)"
									:disabled="!row.active">
