...
import { groupKey, isAutoGeneratedName } from '../core/parse_wikitext';
...
import { getRefContentMap } from "../core/build_wikitext";
...
function prepareRows(refs: Reference[], contentMap?: Map<string, string>): RenameRow[] {
	...
	const rows = refs.map((ref) => {
		...
		// 	});
		// }
		const auto = isAutoGeneratedName(ref.name);
		return {
			ref,
			metadata,
...
export async function openMassRenameDialog(refs: Reference[], options?: MassRenameOptions): Promise<void> {
	...
	const appOptions = {
		...
		methods: {
			...
			/**
			 * Convert wikitext content into link segments, with optional fallback.
			 * @param content - Content to linkify.
			 * @param fallback - Text to display when there is no content.
			 * @returns Array of text/link segments for safe rendering.
			 */
			linkifyOrFallback(this: MassRenameCtx, content: string, fallback: string): LinkifiedSegment[] {
